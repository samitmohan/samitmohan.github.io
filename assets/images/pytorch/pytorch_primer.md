...
- Everything you need to know about PyTorch
...
![PyTorchOverview](pytorchoverview.png)

- Tensor library is very similar to numpy with support for GPU acceleration for math calculations
- The autograd library is where the heart of pytorch is, no need to compute backprop by hand anymore!! PyTorch allows you to save gradients and creates a graph of all gradients of weights and biases so it can update the weights easily. Simplifies backprop and model optimisation.
- The DL library has pretty cool things too, prebuilt loss functions, models, datasets, optimisers which are all used in modern time.
...
False # this would print True if you are using Google Collab GPU or personal GPU. Not on apple silicon chips / CPU (for M-series chips you have print(torch.backends.mps.is_available()) which makes inference really faster)
```

> What is a tensor?

Fancy word for a data containers / arrays
Scalar is a tensor of rank 0, Vector is tensor of rank 1, Matrix is tensor of rank 2.
[0] : scalar (0d tensor)
[1
 2
 3] : 1d tensor (vector)

- its an n-dimensional data structure containing some sort of scalar type, e.g., floats, ints, et cetera.
...
```python
print(f"Matrix 1 : {matrix1} of shape {matrix1.shape}")
matrix2 = matrix1.T 
print(f"Matrix 2 (after transposing matrix1) : {matrix2} of shape {matrix2.shape}")
```

```text
Matrix 1 : tensor(
        [[1, 2, 3],
        [4, 5, 6]]) of shape torch.Size([2, 3])
Matrix 2 (after transposing matrix1) : 
tensor([[1, 4],
        [2, 5],
        [3, 6]]) of shape torch.Size([3, 2])
```

- To multiply 2 matrices -> use @
...
![ag1](autogradmeta.png)
![ag2](autogradops.png)
![ag1](autograd_graph.png)
![ag1](autogradengine.png)
...
def train(dataloader, optimizer, loss_fn, epochs=50, model=model):
    ...
    for epoch in range(epochs):
        ...
        if (batch_no + 1) % 10 == 0:
            ...
            print(f'Epoch {epoch + 1}/{epochs}, Batch {batch_no + 1}/{len(dataloader)}, Loss: {avg_loss:.4f}')

print("Starting training")
train(train_dataloader, optimiser, loss_fn, epochs=50, model=model)
...
class MLP2Hidden(torch.nn.Module):
    def __init__(self, h1=256, h2=128):
        ...
        self.b1 = nn.Parameter(torch.zeros(h1)) # bias of hidden layer 1

        # from hidden layer 1 to hidden layer 2
        self.W2 = nn.Parameter(torch.randn(h1, h2) * 0.01)
        self.b2 = nn.Parameter(torch.zeros(h2)) # bias of hidden layer 2 

        # from hidden layer 2 to output (10 classes in MNIST digits)
        self.W3 = nn.Parameter(torch.randn(h2, 10) * 0.01)
        self.b3 = nn.Parameter(torch.zeros(10)) # bias of output

    def forward(self, x):
        x = x.view(x.size(0), -1) # [B, 784] flatten neurons
...
- how this maps to backprop equation we did
# Backpropagation for a 2-Hidden-Layer MLP (MNIST)

This document maps a PyTorch MLP implementation directly to the
backpropagation equations derived mathematically.
...
- unsqueeze, squeeze, view, slicing, math in pytorch
- More GPU stuff + Diagrams
