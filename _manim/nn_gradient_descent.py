...
class GradientDescent(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(Create(axes), Create(surface), Write(title), run_time=1.5)
        self.wait(0.3)
        ...
        self.play(Create(ball), run_time=0.5)
...
class GradientDescent2D(Scene):
    ...
    def construct(self):
        ...
        curve = axes.plot(lambda x: x**2, color=BLUE_D, x_range=[-3, 3])
        ...
        self.play(Create(axes), Write(x_label), Write(y_label),
                  Create(curve), Write(curve_label), Write(title), run_time=1)
