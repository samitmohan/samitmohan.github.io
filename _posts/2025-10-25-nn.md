...
Introduction to Deep-Learning implementation.

**what this covers:** perceptrons to full neural networks, gradient descent and backpropagation explained with actual math, then building a digit classifier from scratch in python. heavy on intuition, heavier on implementation.
...
Neural networks are behind pretty much every interesting thing happening in AI right now: image recognition, language models, self-driving cars, you name it. Understanding how they work from the ground up gives you a huge advantage because once you get the core ideas, everything else is just variations on the same theme.

This is a long one. There are sections which you can skip if you are not interested in the math.
...
Although the math is quite brilliant & also pretty simple to understand once you really sit down and think about it, it can be intimidating.
...
This is the heart of all algorithms. Every program you will ever code in machine learning / AI will involve these basic chain rule calculus and matrix multiplications at its heart to learn the proper parameters (weights and bias) for your model. 
...
If you have multiple (ALOT) of layers & you're doing this → The value at which the initial weights learn (by the time it gets there) becomes close to 0 so they don't end up learning.
...
Negative Input: If the weighted sum of inputs to a ReLU neuron (before applying the activation function) is negative, the ReLU function will output 0 (definition of $ReLU : f(a) = max(0, a)$)
