...
YOLO uses a CNN-based architecture.
...
> **Problem**: If two objects' centers fall in the same grid cell, YOLO can only detect ONE of them.
>
> **Example**: Dog and bicycle both in cell (3,4) → Model predicts only one object
>
> **Why it happens**: Each cell predicts only ONE set of class probabilities (shared by both boxes)
>
...
```
Recall = TP / (TP + FN) = TP / (number of ground truth objects)
```
High recall = few missed objects

**The trade-off**:
- Predict more boxes → higher recall, lower precision
- Predict fewer boxes → higher precision, lower recall
...
Where:
- `spatial_prior`: Whether the prediction lies in/near the GT box
- `classification_score`: Predicted class confidence
- `IOU`: Overlap between predicted and ground truth boxes
- `α, β`: Hyperparameters balancing classification vs localization

**Why Consistent Matching?**

Using the **same metric** on both branches ensures the o2o head learns from o2m's rich supervision. The supervision gap between the two heads is measured using **1-Wasserstein distance**:

```
Distance = measure of "how much you need to move" to align o2o with o2m targets
```

By minimizing this gap, the o2o head learns to provide **clean, single-box predictions** without needing NMS at inference.

**At Inference:**
- Discard the o2m head completely
- Use only the o2o head → **NMS-free predictions**
- Simply pick top-K class scores → done!
...
**ODN Model (Your Sikkim RTSP Stream Project)**
...
**Inference Pipeline:**
1. **Input**: RGB image from RTSP stream
2. **Preprocessing**: Letterbox resize, normalize to [0,1], convert to CHW format
3. **Model**: YOLOv10m forward pass → tensor [B, N, 4+nc]
4. **Postprocessing**:
   - xywh → xyxy conversion
   - Scale boxes back to original image (undo letterbox padding)
   - Top-K class score selection
   - No NMS needed!
5. **Output**: Clean detections `{boxes: [M,4], scores: [M], labels: [M]}`
