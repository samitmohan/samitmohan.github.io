...
```text
m(α, β) = spatial_prior × classification_score^α × IOU(bbox_pred, bbox_GT)^β
```
...
- `α, β`: Hyperparameters that balance the impact of classification score vs IOU

**Intuition**: The metric `m` scores "how good is this guess" — rewarding predictions that are both the right class (high p) and have good box overlap (high IOU). This ensures both classification accuracy and precise localization matter for matching.
...
- **o2m** → distributes probability mass across **multiple predictions** (explores diverse locations)
- **o2o** → puts all probability mass on a **single prediction** (precise, sharp target)

Naturally, these targets don't look the same — this mismatch is called the **supervision gap**. To measure how "far apart" the two supervision signals are, YOLOv10 uses the **1-Wasserstein distance**:

```text
Distance = "how much sand you need to move" to align distributions
```

**The Formula**: Denoting an instance's largest IoU as u*, and the largest matching scores as m*(o2m) and m*(o2o):

```text
Wasserstein Distance = Σ |target_o2m(i) - target_o2o(i)|
```
...
**Key insight**: By using consistent matching, the best positive sample for the o2m head is also the best for the o2o head. Both heads optimize consistently and harmoniously — the metric ensures you don't need to move much "sand."
...
- Use only the o2o head → **NMS-free predictions**
- Simply pick top-K class scores from [B, N, 4+nc] tensor → done!
...
- Classification is **2.5× heavier** despite regression being more important for accuracy!

**Solution**: Redesign classification head using:

- Two **depthwise separable 3×3 convolutions** (cheap spatial learning)
- One **1×1 convolution** (channel mixing)
- Result: Significantly lower computational cost while maintaining performance

**B. Spatial-Channel Decoupled Downsampling**

**Traditional Approach**:
Single 3×3 conv with stride=2 does both:

- Spatial reduction: H×W → H/2×W/2
- Channel increase: C → 2C
- Cost: **O(9/2 × H×W×C²)** — expensive!

**YOLOv10 Approach**: Decouple the operations:

1. **Pointwise conv (1×1)**: Only changes channels (C → 2C)
2. **Depthwise conv (3×3, stride=2)**: Only reduces spatial dimensions

- New cost: **O(2×H×W×C² + 9/2×H×W×C)** — much cheaper!

**C. Rank-Guided Block Design**

**The Problem**: Not all network stages are equally important — some are redundant.
...
- Use **7×7 large kernel convolutions** in deeper layers (bigger receptive field)
...
4. **Postprocessing** (`postprocess_nms_free` function):
   ...
   - **xywh → xyxy conversion**: Convert center format to corner format
   ...
   - **No NMS needed** — one prediction per object by design!
