...
categories: tech
tokens: "~12.4k"
description: "What if a computer could detect every object in an image by looking just once? The full YOLO story from v1 to v10, with pytorch."
---
...
In 2015, a paper came out with a name: **You Only Look Once (YOLO)**.

The name says it all. What if, instead of looking at an image thousands of times, a computer could just... look once? This post is the story of that idea. This post discusses YOLO, figures out *why* it works, and sees where it stumbles. This also covers the history of YOLO models and how we reached today's state-of-the-art YOLOv10.
...
YOLO was created by Joseph Redmon et al. (2016) to solve this problem: a single network that can perform both box detection and category prediction in one pass.
...
>
> **Intuition**: Like a teacher grading homework - give more points for correct answers (λ_coord=5), fewer points for saying "I don't know" (λ_noobj=0.5).
...
>
> **Geometric intuition**: √ operation compresses large values more than small values, creating a more balanced loss landscape across different object sizes.
...
> **Key Takeaway**: YOLOv1 excels at **sparse, medium-sized objects with typical aspect ratios**. For edge cases, use YOLOv3+ or YOLOv10.
...
YOLOv10 introduces two major innovations: **NMS-free detection** and a **rethought architecture** that balances speed and accuracy more carefully.
...
- **One-to-Many (o2m) Head**: Matches each ground truth object with multiple predictions. Provides a rich supervision signal by exploring diverse object locations. Think of it as: *"I think the cat is here, here, and maybe here."*
- **One-to-One (o2o) Head**: Matches each ground truth object with exactly one prediction. Learns to output precise, single-box answers. *"The cat is exactly here."*
