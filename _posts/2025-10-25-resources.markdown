---
layout: post
title:  "resources"
date:   2025-06-28 14:06:04 +0530
categories: goals
tokens: "~1.5k"
description: "resources."
---

"Academic weapon" or whatever

---

## 2026 Goals

## must:

- [LLM Architectures](https://sebastianraschka.com/llm-architecture-gallery/): implement all; hand-written notes; git repo and a blog to support.
- [Claude Course](https://www.udemy.com/course/ai-engineer-course/learn/lecture/54967113#overview): how to get great at claude
- Fine Tuning, RAG, Agents, LLMOps google tabs
- Read Hands on LLMs by Jay
- Read Building LLMs from scratch by Sebastian
- [Vizuara DL book]
- [HuggingFaces LLM Course](https://huggingface.co/learn/llm-course/chapter1/1)
- Read CUDA for DL by Elliot
- Read GenAI Design book
- Read Pre-training, Post-training, RL handbooks by Elliot
- [ML Interviews Book](https://huyenchip.com/ml-interviews-book/)
  - [ML Interview Questions](https://aman.ai/primers/ai/interview/)
- [ML Systems Book](https://mlsysbook.ai/book/) along with [ML System Design Stanford](https://stanford-cs329s.github.io/)
- [System Design](https://bytebytego.com/courses/machine-learning-system-design-interview/): notes on all, (ML, GenAI, SDE System Design)
  - [Videos](https://www.youtube.com/playlist?list=PLlvnxKilk3aKx0oFua-HTtFf-d_inQ8Qn)
- [Research papers to read](https://github.com/dzyim/ilya-sutskever-recommended-reading)
- [State of RL](https://aweers.de/blog/2026/rl-for-llms/)
- [Frontier Labs Course](https://www.workatafrontierlab.com/courses)
- [System Design SDE](https://medium.com/@shivambhadani_/system-design-for-beginners-everything-you-need-in-one-article-c74eb702540b#cf4c)
  - [GKCS Course LLD, HLD]
- [FastAPI playlist](https://www.youtube.com/playlist?list=PLKnIA16_RmvZ41tjbKB2ZnwchfniNsMuQ)
- [ML Playlist](https://www.youtube.com/playlist?list=PLJf-Umv9fV2O3RyomJhiri_dRcrtgifvC)
- [Stanford Language Modeling](https://www.youtube.com/playlist?list=PLoROMvodv4rOY23Y0BoGoBGgQ1zmU_MT_)
- [UCB Reinforcement Learning](https://www.youtube.com/playlist?list=PLqYmG7hTraZDM-OYHWgPebj2MfCFzFObQ)
- [More LLMOps](https://www.youtube.com/playlist?list=PLQxDHpeGU14CG-wDgZDqFdjsWhWqtDGdi)
- Backend needed for ML Vizuara

## blogs

- Modern LLM Architectures
- Inference Engineering (IE book, Physics of LLMs, Google Tabs)
- Pre-training, Post-training, RL in depth
- ML System Design (based on bbg, genaidesign book)
- Karpathy videos blog
- Programming pearls blog
- CUDA blog (post book and elliot video)
- What I learnt from LLM from scratch + Hands on LLMs
- What i learnt from AI Engineering book
- Actually learning LLMs blog
- Learnings from DDIA + SysDesign gkcs course
- Learnings from OS 3 easy pieces + writing your own OS
- VIT vs CNN for images
- Hutter Prize (AI is just compression) TinyLLMZip project
- Learnings from GenAI handbook
- Agentic AI playbook course review
- Learning a new language (scala)
- Backend for ML

## projects 

  - triton flash attention
  - autoresearch for vision?
  - llmcouncil
  - stable diffusion paper
  - multimodal vision project
  - shelfml + [fastapi](https://www.youtube.com/playlist?list=PLKnIA16_RmvZ41tjbKB2ZnwchfniNsMuQ)
  - [train smol model to write sql](https://diicell.bearblog.dev/teaching-a-smol-model-to-write-sql-queries/)
  - [ai chatbot](https://www.youtube.com/watch?v=KyQKTJhSIak&list=PLJf-Umv9fV2O3RyomJhiri_dRcrtgifvC&index=31&t=43513s)
  - [shazam](https://github.com/ChrisNick92/deep-audio-fingerprinting/tree/main)

## video to watch + books/website to read

- [from tcp to http](https://www.youtube.com/watch?v=FknTw9bJsXM&list=PLJf-Umv9fV2O0ctm_Nrxb9M78waQUnztE&index=20&t=1155s)
- [ProbStats](https://www.youtube.com/playlist?list=PLoROMvodv4rOpr_A7B9SriE_iZmkanvUg)
- [Computer Vision](https://www.youtube.com/playlist?list=PL1HdfW5-F8AQlPZCJBq2gNjERTDEAl8v3)
blogs to write:
- [tcpip for programmers](https://www.youtube.com/watch?v=0OztKsGTqos&list=PLJf-Umv9fV2O0ctm_Nrxb9M78waQUnztE&index=3)
- [git video (primegan)](https://www.youtube.com/watch?v=rH3zE7VlIMs)
- [multi-threading interview questions](https://www.youtube.com/watch?v=GHYFTmeIgVw)
- DDIA
- [neural networks are decision trees](https://arxiv.org/pdf/2210.05189)
- docker bare metals, kubernetes
- Train smol model to write SQL project
- alice adventures in differentiable wonderland
- accidental cto
- functional programming in scala
- annotated turing
- [inside nvidia gpu](https://www.aleksagordic.com/blog/matmul)
- [gpu cornell](https://cvw.cac.cornell.edu/gpu-architecture)
- code by charles 
- statquest guide to ml
- [ml deployment](https://fullstackdeeplearning.com/course/2022/lecture-5-deployment/)
- [what every programmer should know about memory](https://people.freebsd.org/~lstewart/articles/cpumemory.pdf)
- [journey to the stack](https://manybutfinite.com/post/journey-to-the-stack/)
- [singularity](https://web.archive.org/web/20070613184827/http://yudkowsky.net/singularity.html)
- [prime intellect](https://ia903205.us.archive.org/11/items/prime_intellect/prime_intellect.pdf)
