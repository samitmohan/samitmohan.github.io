...
<video autoplay loop muted playsinline style="max-width:100%" poster="/assets/images/math/chain_rule_poster.jpg">
...
that's all the math. once the chain rule clicks, everything else is bookkeeping. the derivatives get messier as the network gets deeper, but the idea is always the same: trace how the loss flows backwards through each operation. pytorch does this automatically - but knowing what it's doing under the hood is what separates debugging from guessing.
