...
In short: **NumPy is the foundational library for numerical computing in Python.** It provides a high-performance, multidimensional array object (`ndarray`) and a vast collection of tools to work with these arrays.
...
NumPy then "stretches" the dimension with size 1 to match the other. In our example:

- `x.shape` is `(4, 3)`
- `v.shape` is `(3,)`
NumPy internally treats `v` as `(1, 3)`.
- Trailing dimensions: `3` and `3` match.
- Next dimensions: `4` and `1` are compatible. NumPy "stretches" `v`'s first dimension to 4, effectively stacking it 4 times to match `x`'s shape.
