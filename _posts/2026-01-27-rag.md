...
categories: tech
tokens: "~4k"
description: "Building a RAG pipeline from scratch - chunking with overlap, hybrid retrieval, streaming generation via Groq, and an evaluation pipeline to actually measure if it works."
---
...
(function(){
  ...
  c.addEventListener('mousemove',function(e){if(!drag)return;var r=c.getBoundingClientRect(),dx=e.clientX-r.left-cx,dy=e.clientY-r.top-cy,m=Math.sqrt(dx*dx+dy*dy);if(m>5){bx=dx/m;by=dy/m;draw();}});
