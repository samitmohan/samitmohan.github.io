...
What even is a RAG? Why can't I just use ChatGPT on my blog? Because ChatGPT doesn't know about samitmohan.github.io; it is a generalised model which can't answer questions related to my blog. For that I could fine-tune a model, but that's expensive and overkill. Instead, I built a RAG pipeline - give the model my blog content as context at query time, no fine-tuning needed.

![finetune](/assets/images/rag/fine-tune.webp)

This post documents how I built it, what worked, what I got wrong the first time, and how I fixed it.
...
Here's the thing about chunking that took me a while to internalize: **your embedding model doesn't care about your paragraph breaks**. It cares about semantic density. My first chunker was dead simple - one paragraph = one chunk. The result? 2851 chunks with a median size of 16 words. Sixteen words. That's barely a sentence.

Think about it - if someone asks "how does KV cache work?", and the answer spans 3 consecutive paragraphs, my old chunker would retrieve maybe 1 of those 3 paragraphs. The embedding for a 16-word chunk is noisy; there's not enough semantic signal for FAISS to work with.
...
The overlap part is important. Without it, if a key sentence falls right at a chunk boundary, you lose it from both chunks. With 50 tokens of overlap, boundary information shows up in both neighboring chunks.

Here's what the core splitting logic looks like:
...
**Result:** Chunk count dropped from **2851 to 1111**. Same 17 blog posts, but now each chunk actually has enough context for the embedding model to produce a meaningful vector. More importantly, retrieval quality went up dramatically (numbers in Phase 5).
...
Both text and code are embedded using the same model, but they are flagged via metadata to distinguish between them during retrieval. There's a subtle detail here - I prefix text chunks with `"Explain the following text:\n"` and code chunks with `"Code example:\n"` before embedding. Same model, but the prefix biases the embedding vector slightly toward the right semantic neighborhood.
...
One thing I normalize vectors before adding them to FAISS. This means L2 distance becomes equivalent to cosine distance, which is what you actually want for semantic similarity. Without normalization, a longer chunk could appear "closer" just because its embedding has larger magnitude, not because it's more relevant.
...
Here's what the golden set looks like:
