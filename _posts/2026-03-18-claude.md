...
I used to type `claude` and just raw dog it. It worked, but I was leaving most of the tool on the table. Since it's the future for the next year at least, might as well learn it end to end. Anthropic has a pretty [cool guide](https://www.anthropic.com/learn/build-with-claude) on how to use claude - these are just my notes while learning.
...
> claude code isn't an autocomplete. it's a while loop with a language model as the condition.
...
> CLAUDE.md is not a README. it's a system prompt that persists across every conversation. treat it like config, not documentation.
...
You can also use /model opusplan which uses Opus for planning and Sonnet for execution. Saves tokens; works pretty well.
...
Your local machine keeps doing all the work - filesystem, tools, MCP servers, everything stays local. The phone is a remote viewer; I would've never imagined editing hyper-parameters of my training loop while walking outside. Some might say I need to get a life.
...
Three extension points worth knowing about.
...
The part that surprised me: skills you can't invoke yourself.
...
> skills are not prompts. they're conditional system prompts with tool restrictions, argument handling, and isolation. treat them like functions, not chat messages.

---

## agents.md - specialized workers

I had Claude review a PR and fix the bugs it found, all in one session. The review was worse because of it - it rushed to solving instead of actually analyzing. So I made a reviewer agent that can read code but can't edit it. The reviews got sharper immediately.
...
> subagents are workers, not assistants. give them a narrow job, restrict their tools, and let them focus. a reviewer that can't edit code will give better reviews than one that can.
...
> plugins are the distribution layer. skills are what you teach claude. agents are who claude becomes.
...
> the model writes what you let it write. if you don't constrain it, you get the default - and the default is slop.
...
Suprising how good claude is; even this blog on how to use claude, is written by claude :P
