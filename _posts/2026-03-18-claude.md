...
Global CLAUDE.md has the rules that apply everywhere: always use uv, no emojis, no em dashes, ban the slop words, read files before editing them. Project CLAUDE.md for this blog adds `bundle exec jekyll serve` as the preview command and documents where things live - `_posts/`, `_manim/`, `assets/images/`. That's it. Two files, maybe 40 lines total.

Daily agents: the code-reviewer runs on every non-trivial change with Write disabled so it can't help itself. The researcher agent has been building up knowledge about this blog's codebase for weeks - it knows that posts live in `_posts/`, animations are generated with Manim, and the dark mode toggle is in `_layouts/default.html`. That context persists across sessions without me re-explaining it. Skills I actually use: `/commit` for commit messages, `/review-pr` before merging anything, and a `pr-summary` skill that runs `gh pr diff` before Claude sees the prompt so it has actual code rather than my description of the code. The `exa` and `context7` MCP servers are always on - when Claude needs to know the current Torch API or check if something exists, it looks it up instead of guessing.
