...
categories: tech
tokens: "~4.2k"
description: "Forking CPython, modifying the grammar, adding a 'nuke' statement, and other things I found digging through Python's C source."
---
...
Python has many implementations (PyPy, Jython, etc.), but the original and most popular is [CPython](https://github.com/python/cpython).
...
This is because C evaluates `-3 < -2 < -1` as `((-3 < -2) < -1)` due to left-to-right associativity.
...
After updating the grammar file with `make regen-pegen` (Python 3.10+ uses pegen; earlier versions used pgen) and running `make`, we can use our new statement:
...
Python doesn't natively support function overloading like C++:
...
DFA visualisation:
![DFA](/assets/images/python/dfa-python.webp){: loading="lazy"}
...
![AST](/assets/images/python/python-ast.webp){: loading="lazy"}
