...
RoPE is sensitive to the exact position of a token. The compression step in MLA involves matrix multiplication, which is not commutative (A×B ≠ B×A). So you can't apply RoPE before compression and expect it to survive - the positional information gets scrambled in the process.
