...
Weight_up_k: (number_of_heads * dimension_heads) × Length  
Weight_up_v: (number_of_heads * dimension_heads) × Length  
...
> This is like a smart traffic controller at a busy intersection. If one road (expert) gets too congested, the controller adjusts the traffic lights (bias) to redirect cars (tokens) to less busy roads. The γ is like a small adjustment, ensuring the change isn't too drastic. This keeps traffic flowing smoothly without creating new jams elsewhere.
...
Let ut​ (Everything the model knows so far about this token) be the embedding of the t-th token coming into the FFN/MoE layer
...
- Most standard language models — like GPT, LLaMA, etc. — are trained with next-token prediction (NTP)
...
- **Denser learning signal**
  ...
  - With multi-token prediction, each position yields n prediction losses simultaneously — i.e., more gradient signal per training position. More signal → faster learning per example

- **Inference acceleration**
  - If you can predict multiple tokens in one pass, you reduce the number of sequential forward passes needed

- **More gradient signal per token**
