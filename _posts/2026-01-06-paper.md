...
categories: tech
tokens: "~4.6k"
description: "How to read and understand machine learning research papers effectively."
---
...
1. **Multi-Head Latent Attention (MLA)** → fixes KV-cache / inference memory
2. **DeepSeekMoE with aux-loss-free balancing** → fixes MoE instability
3. **Multi-Token Prediction (MTP)** → denser training signal + faster inference
4. **FP8 + DualPipe (systems)** → makes all of this feasible at scale
...
![arch](/assets/images/paper/arch.png){: loading="lazy"}
...
<video autoplay loop muted playsinline style="max-width:100%">
  <source src="/assets/images/paper/mla_compression.mp4" type="video/mp4">
</video>
...
- We all know Multi-Head attention works, but it has a huge problem -> KV cache is HUGEEE. How do we reduce this space?
- This is how attention scores look like for a normal MHA
  - ![attention_scores](/assets/images/paper/attention_mha.png){: loading="lazy"}
...
- ![cl](/assets/images/paper/commutative_law.png){: loading="lazy"}
...
- ![drope](/assets/images/paper/decoupledROPE.png){: loading="lazy"}

### Memory Requirements Summary

![comp](/assets/images/paper/comparision.png){: loading="lazy"}
...
<video autoplay loop muted playsinline style="max-width:100%" loading="lazy">
  <source src="/assets/images/paper/moe_routing.mp4" type="video/mp4">
</video>
...
![dm](/assets/images/paper/ffn_dense.png){: loading="lazy"}

**Q. What are sparse models?**
![sm](/assets/images/paper/sparse.png){: loading="lazy"}
![sm2](/assets/images/paper/sm2.png){: loading="lazy"}
...
Token → Attention → Feed-Forward Network (FFN) → Next layer
...
![replace](/assets/images/paper/replaceFFN.png){: loading="lazy"}
...
token_embedding → router → scores for each expert
Then:
...
![moe](/assets/images/paper/moe.png){: loading="lazy"}
...
- Input Encoding
  ...
  - The router’s job is only to decide which experts should handle this input.
...
![moe2](/assets/images/paper/moe2.png){: loading="lazy"}
...
- DeepSeek v3 does Bias-Based Routing for DeepseekMoE
  - ![bias](/assets/images/paper/bias.png){: loading="lazy"}
...
> bias term (γ) is dynamically added or subtracted from an expert's score based on whether it is over- or under-loaded during training
Maintains quality (original affinity scores) while achieving balance (through bias adjustments)

The bias term acts as a lightweight load-balancing mechanism that adjusts routing scores at training time without distorting the learned affinity between tokens and experts. Because the adjustment is small (controlled by γ), routing stays mostly semantic while gradually correcting imbalances.
...
![formula](/assets/images/paper/formula.png){: loading="lazy"}
...
![sparsity](/assets/images/paper/sparsity.png){: loading="lazy"}

**Step 3: Normalize the selected experts (soft weighting)**
![normalise](/assets/images/paper/normalise.png){: loading="lazy"}
> So they sum upto 1 (makes outputs stable, prevents exploding activations)

**Step 4: Apply routed experts**
![routed](/assets/images/paper/routed_exp.png){: loading="lazy"}
...
- Multi-token prediction is basically predicting multiple future tokens at once (e.g., the next 𝑛 tokens) from the same input context
...
- Advantages
  ...
  - ![loss](/assets/images/paper/loss_for_mtp.png){: loading="lazy"}
...
DeepSeek-V3 is an MoE Transformer that reduces active compute per token by routing tokens to a small subset of experts (37B activated vs 671B total). To make routing cheap and performant they (a) compress KV caches via Multi-Head Latent Attention (to shrink inference memory), (b) use a bias-driven routing update to balance expert load without a heavy auxiliary loss, and (c) densify supervision via Multi-Token Prediction (sequentially predicting D future tokens while preserving causal chains). On the infra side they push FP8 training (tile/block quantization + FP32 accumulation windows) and a pipeline scheme (DualPipe) to hide all-to-all communication. Together those engineering choices let them claim strong benchmarks at a lower reported GPU cost
