...
categories: tech
tokens: "~1.1k"
description: "I built an MCP server so Claude can control my Spotify. Play, pause, queue songs by mood - all through natural language."
---

MCP (Model Context Protocol) is Anthropic’s open protocol that lets LLMs actually do things instead of just talking about them. Claude Desktop supports it natively. One standard protocol, any model, any tool. That’s it.

I use Spotify constantly and I use LLMs constantly. Why can’t Claude just control my Spotify? Turns out now it can.
...
The highlights: play/pause/skip, queue songs, create playlists by mood or vibe, get info on what’s playing, shuffle/repeat, pull up lyrics (via Genius API), and browse your liked songs and recently played. Full Spotify control through natural language.
...
I was surprised by how fast this came together. Spotipy handles all the OAuth and API calls, so the MCP layer itself is minimal.
...
MCP changes that. LLMs can now take real actions instead of just generating text.

Next up: a trading bot that talks to Zerodha, or a flight booking tool so I don't have to open MMT. Both should be straightforward now. I can just tell Claude to do it.
